...
\item Real Words
   ...
   \end{enumerate}
   \subsection{3c}
Removing stop words might make sense if it is only by chance that there is a large discrepency in their appearence in fake or real lines. In that case, the model overfts. It might make senes to keep these words if they trully have real predictive power, that is,
over enough samples they are shown to be more likely to appear in one class of headline over another.
